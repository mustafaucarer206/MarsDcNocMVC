# PostgreSQL'den tum salon ve cihaz kombinasyonlariyla filmleri eksiksiz aktar
param(
...
Write-Host "`n=== EKSIKSIZ SALON VE CIHAZ AKTARIMI ===" -ForegroundColor Magenta
Write-Host "Tum projector ve SMS salonlari dahil`n" -ForegroundColor Cyan
...
# PostgreSQL'den eksiksiz sorgu - tum salon ve cihaz kombinasyonlari
$query = @"
SELECT 
    ...
    CASE 
        ...
        -- Projector salonlari (detayli)
        WHEN s.title = 'Solaria / SR1000' AND d.category = 'projector' THEN 'Salon 1 - Solaria SR1000'
        WHEN s.title = 'Solaria / IMB-S2' AND d.category = 'projector' THEN 'Salon 2 - Solaria IMB-S2'
        WHEN s.title = 'CP2230U / IMB-S2 - MPX' AND d.category = 'projector' THEN 'Salon 3 - CP2230U IMB-S2'
        WHEN s.title = 'CP2210 / SR1000' AND d.category = 'projector' THEN 'Salon 4 - CP2210 SR1000'
        WHEN s.title = 'CP2220 / SR1000' AND d.category = 'projector' THEN 'Salon 5 - CP2220 SR1000'
        WHEN s.title = 'CP2230 / SR1000' AND d.category = 'projector' THEN 'Salon 6 - CP2230 SR1000'
        WHEN s.title = 'Solaria/ SR1000' AND d.category = 'projector' THEN 'Salon 7 - Solaria SR1000'
        -- SMS salonlari (detayli)
        WHEN s.title = 'Solaria / SR1000' AND d.category = 'sms' THEN 'SMS 1 - Solaria SR1000'
        WHEN s.title = 'Solaria / IMB-S2' AND d.category = 'sms' THEN 'SMS 2 - Solaria IMB-S2'
        WHEN s.title = 'CP2230U / IMB-S2 - MPX' AND d.category = 'sms' THEN 'SMS 3 - CP2230U IMB-S2'
        WHEN s.title = 'CP2210 / SR1000' AND d.category = 'sms' THEN 'SMS 4 - CP2210 SR1000'
        WHEN s.title = 'CP2220 / SR1000' AND d.category = 'sms' THEN 'SMS 5 - CP2220 SR1000'
        WHEN s.title = 'CP2230 / SR1000' AND d.category = 'sms' THEN 'SMS 6 - CP2230 SR1000'
        WHEN s.title = 'Solaria/ SR1000' AND d.category = 'sms' THEN 'SMS 7 - Solaria SR1000'
        -- Genel kategoriler
...
Write-Host "PostgreSQL'den eksiksiz salon ve cihaz bilgileriyle filmler cekiliyor..." -ForegroundColor Gray
...
foreach ($row in $filmData) {
    if ($row -is [string] -and $row -match '\|') {
        try {
            ...
            $salon = if($fields[8]){($fields[8] -replace "'", "''")}else{'Diger'}
...
WITH DuplicateMovies AS (
    SELECT Id, ROW_NUMBER() OVER (PARTITION BY FilmAdi, SalonAdi ORDER BY Id) as rn
    FROM Movies
)
DELETE FROM Movies WHERE Id IN (SELECT Id FROM DuplicateMovies WHERE rn > 1);
"@
...
Write-Host "Artik tum projector ve SMS salonlari ayri ayri gorunuyor!" -ForegroundColor Cyan
